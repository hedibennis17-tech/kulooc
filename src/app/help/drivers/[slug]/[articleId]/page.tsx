...
export default function DriverArticlePage({ params }: { params: { slug: string, articleId: string } }) {
    const article = helpData.articles.find(a => a.id === params.articleId);
    const section = driverHub?.sections.find(s => s.label === article?.section);
    ...
    return (
        <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div className="mb-8">
                <Link href={`/help/drivers/${section.key}`} className="flex items-center text-sm text-muted-foreground hover:text-primary mb-2">
                    <ChevronLeft className="h-4 w-4 mr-1" />
