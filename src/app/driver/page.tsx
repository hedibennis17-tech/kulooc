'use client';
/**
 * KULOOC — Page Chauffeur v8
 * ────────────────────────────────────────────────────────────────────────────
 * v8 :
 *  1. Supprimé : bloc gains/courses dans le panneau bas (hors ligne/en attente)
 *  2. Supprimé : bloc blanc étapes numérotées (2/3/4/5) pendant la course active
 *  3. Conservé : GPS manuel, modal évaluation, ping driver, dispatch V2 intact
 *  4. GPS plein écran — NavigationMap intégré (jamais de popup externe)
 */
...
export default function DriverHomePage() {
  ...
  // ── Moteur dispatch V2 — démarre dès auth ─────────────────────────────────
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-gray-100 flex flex-col">
      ...
      {/* ══════════════════════════════════════════════════════════════
          ...
          (bloc étapes numérotées SUPPRIMÉ — boutons directs)
      ══════════════════════════════════════════════════════════════ */}
      {activeRide && (
        <div
        ...
        >
          ...
          <div className="px-5 pb-6 space-y-3">
            ...
            {/* Bouton GPS — driver-assigned + GPS pas encore actif */}
            ...
            {/* Indicateur GPS actif + Je suis arrivé */}
            {(activeRide.status as string) === 'driver-assigned' && isNavActive && (
            ...
            {/* Commencer la course — driver-arrived */}
            {(activeRide.status as string) === 'driver-arrived' && (
              <div className="space-y-3">
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-amber-800">En attente du passager</p>
            ...
            {activeRide.passengerPhone && ((activeRide.status as string) === 'driver-assigned' || (activeRide.status as string) === 'driver-arrived') && (
              <a href={`tel:${activeRide.passengerPhone}`}
                className={`w-full py-3 rounded-full border font-semibold text-sm flex items-center justify-center gap-2 ${
                  isNavActive ? 'border-white/20 text-white/70' : 'border-gray-200 text-gray-600'
                }`}>
