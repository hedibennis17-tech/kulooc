'use client';
/**
 * KULOOC — ActiveRidesPanel
 * Directive 5 : bouton "Forcer fin de course" pour le dispatcher
 */
...
interface ActiveRidesPanelProps {
  ...
  onForceEnd?: (rideId: string, driverId: string) => Promise<{ success: boolean; error?: string }>;
}
...
export function ActiveRidesPanel({ rides, onUpdateStatus, onForceEnd }: ActiveRidesPanelProps) {
  if (rides.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3 overflow-y-auto h-full pr-1">
      {rides.map((ride) => {
        ...
        const driverId = (ride as any).driverId || '';
        return (
          <div key={ride.id} className="p-4 bg-white rounded-xl border border-gray-100">
            ...
            <div className="mt-2 flex gap-2">
              ...
              {/* Directive 5 : Forcer fin de course — disponible pour tous les statuts */}
              {onForceEnd && ride.id && (
                <button
                  onClick={async () => {
                    if (!window.confirm(`Forcer la fin de la course de ${ride.driverName} → ${ride.passengerName} ?`)) return;
                    const result = await onForceEnd(ride.id!, driverId);
                    if (!result.success) {
                      alert(`Erreur : ${result.error}`);
                    }
                  }}
                  className="flex-1 text-xs py-1.5 rounded-lg bg-red-50 text-red-700 font-medium hover:bg-red-100 flex items-center justify-center gap-1 border border-red-200"
                >
                  <AlertTriangle className="w-3 h-3" /> Forcer fin
                </button>
              )}
            </div>
          </div>
